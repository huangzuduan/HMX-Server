...
private:
	...
	const uint32_t		m_nBlockSize;				// 内存块大小 
	const uint32_t		m_nBlockNumPerSlice;		// 内存片中内存块数量 
...
{
	bool		bUsing;			// 正在使用 
	uint64_t		nMark;			// 内存标记(同一块内存分配给不同对象时标记不同，永不为0,目前为内存使用次数) 
};
...
private:
	...
	int32_t m_Interval;	// 每次间隔时间(毫秒) 
	int64_t m_execTime;	// 已经执行的时间 
...
public:
	...
	int32_t status;				// 状态
	int32_t lastasktime;		// 上次访问时间 
	int32_t lastaskprotocol;	// 上次访问协议
...
public:
	...
	// 将消息发到某区
	void sendToServerZone(GeneralRoute_t route, const NetMsgSS* cmd, int32_t len); // S协议
	void sendToServerZoneUser(GeneralRoute_t route, int64_t toUserID, const NetMsgSS* cmd, int32_t len); //c协议
...
private:
	int32_t						m_nFlagsRow;		// 标志行(1:跳过不读取,2:读取,3:读取) 
	int32_t						m_nDataStartRow;	// 数据起始行 
	uint32_t					m_nCurrRowIndex;	// 当前读取到的行[0,xx) 
	...
	uint32_t					m_nCol;				// 当前读取行列索引 
	int32_t						m_nUseColCount;		// 可用列数量 
};
