...
typedef void(*BuffEffectFunc)(SceneNpc* /* 施法者 */, SceneNpc* /* 被施法者 */, EffectAimType/* 目标类型 */, EffectRangeType/* 范围类型 */,
	int32_t/* 参数 */, int32_t/* 距离 */, float32/* 角度cos值 */,int32_t nEffectNum/* 最多影响数量 */);

struct BuffEffect
{
	BuffEffectType		eType;					// 效果类型 
	//int32				nType;
	int32_t				nLimitFlag;				// 效果限制使用标记(BuffFlag二进制位运算值) 
...
struct BuffData
{
	...
	int32_t		nLimitFlag;						// 使用标记 
	int32_t		nLifeTime;						// 生命周期(单位:毫秒) 
	int32_t		nEffectNum;						// 最多打到的生物体 
	struct Effect
	{
		BuffEffectType		nEffectID;			// 效果ID 
		int32_t				nProbability;		// 出现概率(0为一定出现) 
		EffectAimType		eAimType;			// 作用对象类型 
		EffectRangeType		eRangeType;			// 作用范围类型 
		int32_t				nDist;				// 距离 
		float32				fCosAngle;			// 角度(cos值) 
		int32_t				nParam;				// 参数值 
		int32_t				nTimePoint;			// 效果生效时间点(单位:毫秒) 
