...
zTextCVS::zTextCVS(const std::string& strFileName, uint32_t nFlagsRow, uint32_t nDataStartRow)
...
bool zTextCVS::NextRow(int32_t nLines)
{
...
bool zTextCVS::GetInt8(int8_t& o_nValue)
{
	while (m_nCol < m_vecCol.size())
	{
		if (m_vecFlags[m_nCol++])
		{
			o_nValue = static_cast<int8_t>(atoi(m_vecCol[m_nCol - 1].c_str()));
			return true;
		}
	}
...
bool zTextCVS::GetUInt8(uint8_t& o_nValue)
{
	while (m_nCol < m_vecCol.size())
	{
		if (m_vecFlags[m_nCol++])
		{
			o_nValue = static_cast<uint8_t>(atoi(m_vecCol[m_nCol - 1].c_str()));
			return true;
		}
	}
...
bool zTextCVS::GetInt16(int16_t& o_nValue)
{
	while (m_nCol < m_vecCol.size())
	{
		if (m_vecFlags[m_nCol++])
		{
			o_nValue = static_cast<int16_t>(atoi(m_vecCol[m_nCol - 1].c_str()));
			return true;
		}
	}
...
bool zTextCVS::GetUInt16(uint16_t& o_nValue)
{
	while (m_nCol < m_vecCol.size())
	{
		if (m_vecFlags[m_nCol++])
		{
			o_nValue = static_cast<uint16_t>(atoi(m_vecCol[m_nCol - 1].c_str()));
			return true;
		}
	}
...
bool zTextCVS::GetInt32(int32_t& o_nValue)
{
	while (m_nCol < m_vecCol.size())
	{
		if (m_vecFlags[m_nCol++])
...
bool zTextCVS::GetUInt32(uint32_t& o_nValue)
{
	while (m_nCol < m_vecCol.size())
	{
		if (m_vecFlags[m_nCol++])
		{
			o_nValue = static_cast<uint32_t>(ATOL(m_vecCol[m_nCol - 1].c_str()));
			return true;
		}
	}
...
bool zTextCVS::GetInt64(int64_t& o_nValue)
{
	while (m_nCol < m_vecCol.size())
	{
		if (m_vecFlags[m_nCol++])
...
bool zTextCVS::ReadFlagsRow()
{
	...
	// 设置标记列 
	for (uint32_t i = 0; i < m_vecCol.size(); ++i)
	{
