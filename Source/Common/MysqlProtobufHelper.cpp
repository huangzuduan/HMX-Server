...
namespace db
{
	...
	std::string ProtoMysqlString(MYSQL *mysql, const google::protobuf::Message& msg)
	{
		if (NULL == mysql)
		{
			return "";
		...
		std::string strPro;
		msg.SerializeToString(&strPro);

		std::stringstream streamPro;
		streamPro << strPro;

		std::string strData = "\"" + realEscape(mysql, streamPro.str()) + "\"";

		return strData;
	}

	std::string StringMysqlString(MYSQL *mysql, const std::string& str)
	{
		std::string strCopy = str;
		//StringUtils::replaceAll(strCopy, "'", "*");

		std::string strData = "\"" + realEscape(mysql, strCopy) + "\"";
